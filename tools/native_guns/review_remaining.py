...
def review():
    ...
    for gun in GUNS:
        ...
        target=read(RES/f'assets/tacz_fork_tarkov/display/guns/{gun}.json')
        ...
        ns,path=index['data'].split(':');assert read(SRC/'data'/ns/'data/guns'/(path+'.json'))==read(RES/f'data/tacz_fork_tarkov/data/guns/{gun}.json'),gun
        metadata={b['name']:{k:v for k,v in b.items() if k!='cubes'} for b in original['bones']}
        counts=[]
        for lod in ('','lod/'):
            geo=read(RES/f'assets/tacz_fork_tarkov/geo_models/gun/{lod}{gun}.json')['minecraft:geometry'][0];bones={b['name']:b for b in geo['bones']}
        ...
        assert read(RES/f'data/tacz_fork_tarkov/{gun}/native-visual-rules.json')==cfg['visualRules']
        assert read(RES/f'data/tacz_fork_tarkov/{gun}/native-profile.json')==cfg['nativeProfile']
        report=read(root/'build-report.json');atlas=Image.open(RES/f'assets/tacz_fork_tarkov/textures/gun/{gun}.png').convert('RGBA');unit=report['atlasUnit']
