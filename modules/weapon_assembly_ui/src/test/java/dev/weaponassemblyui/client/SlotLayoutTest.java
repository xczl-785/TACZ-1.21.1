...
class SlotLayoutTest {
    ...
    @Test void coincidentMountsStillHaveDistinctClickableCards() {
        ...
        check(place(anchors,80),20,80);
    }
    @Test void adjoiningDropdownBarsDoNotCoverNeighboringCards() {
        var anchors=new ArrayList<SlotLayout.Anchor>();
        for(int i=0;i<12;i++)anchors.add(anchor(i,640,348));
        // Each slot reserves the entire 80px card plus its 18px hover bar.
        check(place(anchors,98),12,98);
    }
    @Test void rotationAndExtremeZoomKeepEveryCardInsideTheWorkbench() {
        ...
        for(int step=0;step<72;step++) {
            ...
            var positions=state.update(anchors,AREA,80,80,Set.of(),1d/60);check(positions,12,80);
        }
    }
    @Test void locationComesFromMountProjectionRatherThanListParity() {
        var left=place(List.of(anchor(0,100,348)),80).get(List.of("receiver","slot0"));
        var right=place(List.of(anchor(0,1180,348)),80).get(List.of("receiver","slot0"));
    ...
    @Test void sameInputHasDeterministicLayout() {
        ...
        assertEquals(place(anchors,80),place(anchors,80));
    }
}
