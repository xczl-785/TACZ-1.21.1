...
独立组装工作台，依赖 `weapon_assembly` 算法与 `foundation` 公共 LDLib2 控件。按 Gunsmith 原型保留部位标签、附近浮窗、悬停预览、点击安装、拆卸、撤销、恢复初始配置、相机旋转与复位；通过“进入子槽 / 返回上层”处理递归配件。完整产品链、权威边界和分阶段验收见 [组装台完整方案](docs/组装台完整方案.md)。
...
5. 拖空白处旋转；候选列表内的滚轮只滚动列表，画布不做用户缩放或平移；复位视角只改变相机。
