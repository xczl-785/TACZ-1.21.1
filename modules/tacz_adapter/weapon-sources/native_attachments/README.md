...
已完成独立编辑源、隔离派生模型/纹理、来源和兼容清单，并随NewMod制品打包。后继批次已由十五枪作者配置显式消费，实际绑定以各枪 `production.json` / M4 `assembly.json` 为准；不写全局原生 display。原版配件退役和身份替换尚未执行，不能把独立模型制作等同于旧物品入口已退出。
