...
public final class NativeAssemblyIconRaster {
    ...
    public static int[] bake(AssemblyNode root,Map<String,ModelGeometry> models,AssemblyMaterials materials,Function<String,Texture> textures,int size){
        return bake(root,models,materials,textures,size,size);
    }
    public static int[] bake(AssemblyNode root,Map<String,ModelGeometry> models,AssemblyMaterials materials,Function<String,Texture> textures,int width,int height){
        if(width<16||height<16||width>2048||height>2048)throw new IllegalArgumentException("Invalid icon size");
        var faces=new ArrayList<Face>();collect(root,root,List.of(),models,materials,faces);
        int[] pixels=new int[width*height];if(faces.isEmpty())return pixels;
        ...
        double scale=.90*Math.min(width/Math.max(1e-6,maxX-minX),height/Math.max(1e-6,maxY-minY)),cx=(minX+maxX)/2,cy=(minY+maxY)/2;
        ...
        for(var face:faces){
            ...
            for(int i=0;i<3;i++){var p=triangle.vertices().get(i).add(face.offset);x[i]=(horizontal(p)-cx)*scale+width/2.;y[i]=(-p.y()-cy)*scale+height/2.;z[i]=depth(p);}
            ...
            for(int py=y0;py<=y1;py++)for(int px=x0;px<=x1;px++){
                ...
                int index=py*width+px;double distance=w0*z[0]+w1*z[1]+w2*z[2];if(distance<=zbuffer[index])continue;
    ...
    public static float[] bounds(int[] pixels,int size){
        return bounds(pixels,size,size);
    }
    public static float[] bounds(int[] pixels,int width,int height){
        if(width<1||height<1||pixels.length!=width*height)throw new IllegalArgumentException("Invalid icon pixels");
        int minX=width,minY=height,maxX=-1,maxY=-1;
        for(int y=0;y<height;y++)for(int x=0;x<width;x++)if((pixels[y*width+x]>>>24)!=0){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);}
        if(maxX<0)return new float[]{-8,-8,8,8};
        return new float[]{16f*minX/width-8,16f*minY/height-8,16f*(maxX+1)/width-8,16f*(maxY+1)/height-8};
    }
