...
class AdoptedAmmunitionTest {
    @Test void approvedRoundsRetainSourceParametersAndStorageProfiles() throws Exception {
        var entries=AmmunitionContent.load();assertEquals(86,entries.size());assertEquals(12,entries.stream().map(AmmunitionContent.Entry::caliber).distinct().count());
        try(var in=AmmunitionContent.class.getResourceAsStream("/data/tarkov_content/catalog/ammunition.json")) {
            ...
            for(int i=0;i<rows.size();i++) {
                ...
                try(var profile=AmmunitionContent.class.getResourceAsStream("/data/tarkov_content/item_foundation/items/ammo_"+entry.sourceId()+".json")) {
                ...
                assertNotNull(AmmunitionContent.class.getResource("/assets/tarkov_content/textures/item/ammo_"+entry.sourceId()+".png"));
            }
        }
    }
}
