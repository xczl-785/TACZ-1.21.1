...
原独立 adapter 已取消；构建与安装请从[第二项交付](../../../docs/assembly-experiment/adapter合并交付.md)进入。

实验室只发布一个 tacz Mod。正式包包括原 adapter 正式代码和三枪；开发验收包在同一包内增加开发工具。NewMod 宿主只选择其中一个完整 Jar，同时加载独立公共玩法 Mod，不发布宿主 Jar。
